...
def level_force_directed(
	...
	for iteration in range(start_index, end_index):
		force_event = force_kernel(commands, (globalWorkSize,), (localWorkSize,), d_highD, d_weights, d_lowD_a, d_lowD_b, d_velocity_a, d_velocity_b, d_force, d_seed_memory, d_pivot_indices, 
...
def reduce(data, weights, dimensions=2):
	#print(weights)
	
	num_of_points, n_original_dims = data.shape
	highD = np.array(data.flatten()).astype(np.float32)
	weights = np.array(weights).astype(np.float32)

	near_set_size = 4
	random_set_size = 4
	n_projection_dims = dimensions
	group_size = 128
        if num_of_points < 128:
            group_size = 64
	...
	# Load Kernels
	#program = cl.Program(context, open("claret/helper/claret_kernel.cl").read()).build()
	#print(sys.path[0])
	program = cl.Program(context, open(os.path.join(sys.path[0], "../Nebula-Pipeline/nebula/model/claret", "claret_kernel.cl")).read()).build()
	force_kernel = program.compute_weighted_force
	...
	d_weights = cl.Buffer(context, mf.READ_ONLY | mf.COPY_HOST_PTR, size = weights.nbytes, hostbuf=weights)
	...
	while g_done == False :
		if g_interpolating == True: # interpolate
			...
			num_of_groups = np.int32(math.ceil(g_heir[g_current_level] / float(group_size)))
	...
	projection = lowD.reshape((num_of_points, n_projection_dims))
	#print("Projection time: " + str(et - st))
	return projection
