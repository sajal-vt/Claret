...
m __future__ import absolute_import, print_function
import numpy as np
import pyopencl as cl
                                num_of_points,
                                n_original_dims);
...
bool testLowDistance(int pointIdx, float* lowD, float* pivot, float* output)
{
	for(int i = 0; i < 24; i++)
	{
		...
		std::cout << dist from __future__ import absolute_import, print_function
import numpy as np
import pyopencl as cl<< " <> " << output[pointIdx * 24 + i] << std::endl;
	}
}
