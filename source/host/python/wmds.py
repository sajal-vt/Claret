...
def wmds(highD, weights, num_of_points, n_original_dims):
	...
	projection_writer = open("projection.csv", "w")
...
def main(argv):
	...
	highD = np.array(highD).astype(np.float32)
	weights = np.array(np.ones(n_original_dims)).astype(np.float32)
	print(np.shape(highD))

	wmds(highD, weights, num_of_points, n_original_dims)
