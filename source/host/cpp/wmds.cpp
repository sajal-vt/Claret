...
int main(int argc, char** argv)
{
    ...
    float* weights = new float[n_original_dims];
...
//        std::cout << lowD[i] << std::endl;
	...
	force_kernel = clCreateKernel(program, "compute_weighted_force", &err);
	...
	d_weights = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * n_original_dims, NULL, &err);
	...
	if (!d_highD || !d_weights || !d_lowD_a || !d_lowD_b || !d_velocity_a || !d_velocity_b || !d_force || !d_pivot_indices || !d_seed_memory || !d_hd_distances || !d_ld_distances || !d_metadata)
	{
	...
	err |= clSetKernelArg(force_kernel, 1, sizeof(cl_mem), (void *)&d_weights);
	err |= clSetKernelArg(force_kernel, 2, sizeof(cl_mem), (void *)&d_lowD_a);
	err |= clSetKernelArg(force_kernel, 3, sizeof(cl_mem), (void *)&d_lowD_b);
    err |= clSetKernelArg(force_kernel, 4, sizeof(cl_mem), (void *)&d_velocity_a);
    err |= clSetKernelArg(force_kernel, 5, sizeof(cl_mem), (void *)&d_velocity_b);
	err |= clSetKernelArg(force_kernel, 6, sizeof(cl_mem), (void *)&d_force);
	err |= clSetKernelArg(force_kernel, 7, sizeof(cl_mem), (void *)&d_seed_memory);
	err |= clSetKernelArg(force_kernel, 8, sizeof(cl_mem), (void *)&d_pivot_indices);
	err |= clSetKernelArg(force_kernel, 9, sizeof(int), (void *)&start_index);
	err |= clSetKernelArg(force_kernel, 10, sizeof(int), (void *)&end_index);
	err |= clSetKernelArg(force_kernel, 11, sizeof(int), (void *)&n_original_dims);
	err |= clSetKernelArg(force_kernel, 12, sizeof(float), (void *)&n_projection_dims);
	err |= clSetKernelArg(force_kernel, 13, sizeof(float), (void *)&near_set_size);
	err |= clSetKernelArg(force_kernel, 14, sizeof(float), (void *)&random_set_size);
	err |= clSetKernelArg(force_kernel, 15, sizeof(cl_mem), (void*)&d_hd_distances);
	err |= clSetKernelArg(force_kernel, 16, sizeof(cl_mem), (void*)&d_metadata);
	...
	err &= clEnqueueWriteBuffer(commands, d_weights, CL_TRUE, 0, sizeof(float) * n_original_dims, weights, 0, NULL, NULL);
...
for(int iteration = start_index; iteration < end_index; iteration++)
	{
		err |= clSetKernelArg(force_kernel, 17, sizeof(int), (void *)&iteration_pp);
